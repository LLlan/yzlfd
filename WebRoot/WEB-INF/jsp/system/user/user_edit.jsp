	...
	<form action="user/${msg }.do" name="userForm" id="userForm" method="post">
		...
		<table class="table table-striped table-bordered">
			...
			<tr>
				<td>职位：</td>
				<td>
				<select class="chzn-select" name="ROLE_ID" id="role_id" data-placeholder="请选择职位" style="vertical-align:top;width: 290px;">
				<option value=""></option>
		...
		<%-- 	<tr>
				<td>物业公司：</td>
				<td>
				<select class="chzn-select" name="estate_company_id" id="estate_company_id" data-placeholder="请选择物业公司" style="vertical-align:top;width: 290px;">
				<option value=""></option>
				<c:forEach items="${estateList}" var="estate">
					<option value="${estate.estate_company_id }" <c:if test="${estate.estate_company_id == pd.estate_company_id }">selected</c:if>>${estate.estate_company_name }</option>
				</c:forEach>
				</select>
				</td>
			</tr> --%>
			...
			<tr>
			<td>物业公司：</td>
				<td><input type="text" readonly="readonly" value="${pd.estate_company_name }"/></td>
			</tr>
			</c:if>
