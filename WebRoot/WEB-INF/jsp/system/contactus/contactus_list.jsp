...
<script type="text/javascript">
	...
	function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>api/shangHu/toAddContactUs.do';
		 diag.Width = 350;
		 diag.Height = 300;
	...
	function edit(tagID){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑";
		 diag.URL = '<%=basePath%>api/shangHu/toEditContactUs.do?contactus_id='+tagID;
		 diag.Width = 350;
		 diag.Height = 300;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	function del(tagID){
		bootbox.confirm("确定要删除该条数据吗?", function(result) {
			if(result) {
				top.jzts();
				var url = '<%=basePath%>api/shangHu/deleteContactUsById.do?ids='+tagID;
...
<div class="row-fluid">
	<!-- 检索  -->
	<form action="<%=basePath%>api/shangHu/contactuslistPage.do" method="post">
	<!-- 检索  -->
