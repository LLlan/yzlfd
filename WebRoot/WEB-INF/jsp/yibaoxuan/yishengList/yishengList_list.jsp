...
<div class="row-fluid">
	<!-- 检索  -->
	<form action="<%=basePath%>api//getyishenglistPage.do?identity=${msg }" method="post" id="myForm">
	<table>
		<tr>
			<td>
...
<script type="text/javascript">
	...
	function abc(obj,identity){
		...
		window.location.href="api//getyishenglistPage.do?identity="+identity+"&renZhengState="+renZhengState;
	}
	...
	function topp(tagID,topp,msg){
		bootbox.confirm(msg, function(result) {
			if(result) {
				top.jzts();
				var url = '<%=basePath%>api//topShangHu.do?user_shanghu_id='+tagID+'&top='+topp;
		...
		bootbox.confirm(msg,function(result){
			if(result){
				...
				if(str==''){
					...
					$.ajax({
						type:'post',
						url:'<%=basePath%>api//yishengChenked.do',
						dataTyoe:'json',
						data:{
							"ids":str,
	...
	function changrenzhengstate(obj){
		...
		if(value=='1'){
			...
			bootbox.confirm(msg, function(result) {
				if(result) {
					top.jzts();
					$.post('<%=basePath%>api//yishengChenked.do',{ids:ids,renZhengState:renZhengState},function(data){
						nextPage('${page.currentPage}');
					});
				}
			});
	...
	function changstate(obj){
		...
		bootbox.confirm(msg, function(result) {
			if(result) {
				top.jzts();
				$.post('<%=basePath%>api//yishengStatus.do',{id:id,status:status},function(data){
					nextPage('${page.currentPage}');
				});
			}
		});
	...
	function lookBaseInformation(id){
		//获取基本信息
		$.post('api//getBaseInformation.do',{user_shanghu_id:id},function(data){
			var str='';
			var pd=data.pd;
	...
	function lookRenZhengInformation(id){
		$.post('api//getRenZhengInformation.do',{user_shanghu_id:id},function(data){
