...
<!--主体开始-->
<div class="jifenBox">
    <span><span id="myjifen" style="margin-left: 60px;">${pd.integral }</span><font>分</font></span>
    <span>积分可以在平台上消费</span>
     <span>${jifenData.jifen}分</span>
</div>
<c:if test="${pd.jifen_num!=null && pd.jifen_num!='' }">
...
<script type="text/javascript">
	function duihuan(){
		...
		if(parseFloat(myjifen) >= parseFloat(jifennum)){
			$.post('api/shangHu/jiFenHuanGuQuan.do',{},function(data){
				if(data.respCode=='00'){
					...
					setTimeout("window.location.href='api/shangHu/toPersonMyJiFen.do'", 2000);
				}
			});
		}else{
