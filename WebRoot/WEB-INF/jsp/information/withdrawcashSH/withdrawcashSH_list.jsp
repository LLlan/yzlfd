...
<div class="row-fluid">
	<form action="<%=basePath%>api/shangHu/getSHWithDrawCashlistPage.do" method="post" id="myForm">
		<input type="hidden" value="${pd.state }" name="state">
		<table>
			<tr>
				<td>
					<span class="input-icon">
						<input autocomplete="off" id="nav-search-input" type="text" name="searchName" value="${pd.searchName }" placeholder="这里输入流水号" />
			...
			<c:choose>
				<c:when test="${not empty list}">
					...
					<tr>
						...
						<td class='center'>
							<c:choose>
								<c:when test="${var.identity=='1'}">医生</c:when>
								<c:when test="${var.identity=='2'}">机构</c:when>
								<c:otherwise>药商</c:otherwise>
							</c:choose>
						</td>
						...
						<c:choose>
							<c:when test="${var.state=='2' }">
								...
								<td class='center'>
									<a href="javascript:void(0)" class='btn btn-mini btn-info' onclick="chuli('${var.withdraw_cash_id }');">处理</a>
								</td>
							</c:when>
							<c:otherwise>
...
<div id="submitInformation" style="display: none;margin: 25px 0 0 20px;">
	<form action="api/shangHu/updateWithDrawCashState.do" method="post" enctype="multipart/form-data" id="updateForm">
		<input type="hidden" value="" name="withdraw_cash_id" id="withdraw_cash_id">
		<input type="hidden" value="2" name="type">
		图片(转账图片)：<input style="width: 76px;" type="file" name="imgPatnFile" id="imgPatnFile" accept="images/jpg,images/jpeg,images/png,images/bmp"><br><br>
