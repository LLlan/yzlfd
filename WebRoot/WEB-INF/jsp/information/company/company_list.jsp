			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
                        ...
                        <th>普通推广</th>
                        <th>商家状态</th>
						<th><i class="icon-time hidden-phone"></i>添加时间</th>
						<th>商家图片</th>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
								        <td>
									        <a href="${var.logo}" title="${var.name}" class="bwGal"><img src="${var.logo}" alt="${var.name}" width="60"></a>
								        </td>
										<td>${var.name}</td>
										<td>${var.cate}</td>
										<td>${var.salestime}</td>
								        ...
								        <td>
									        <c:if test="${var.status == '0' }"><span class="label label-important arrowed-in">冻结</span></c:if>
									        <c:if test="${var.status == '1' }"><span class="label label-success arrowed">正常</span></c:if>
								        </td>
										<td>${var.time}</td>
										<td><a href="comimages/imagesList?COMID=${var.id}">查看/修改</a></td>
