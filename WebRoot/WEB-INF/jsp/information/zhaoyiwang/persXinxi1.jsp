...
<head>
    ...
    <link rel="stylesheet" href="static/zhaoyiwang/css/demos.css">
    <title>个人信息</title>
    <style>
        body{
...
<div class="wy-header header" style="width:100%;position: fixed;top:0;left:0;z-index: 100;">
    <div class="wy-header-icon-back" >
    	...
    	<!--<span style="position:relative;top:-24px;left:35px;color:#fff;font-size: 16px;">返回</span>-->
    </div>
...
<div class="Pxinxi">
    <ul>
        ...
        <c:if test="${pd.identity!='4' && pd.identity!='5'  }">
	        <li>
	        	<label for="biaozhu">标注</label>
	        	<input type="text" style="margin-left: 5%;" placeholder="个人还是国营" id="biaozhu" name="mark" value="${pd.mark }"/>
	        </li>
	        <li>
	        	<label for="linkMethod">服务热线</label>
	        	...
	        	<c:otherwise>
	        		<li>
			        	...
			        	<select name="category" style="margin-left: 5%;border: 1px solid #dac5c5;height: 30px;width: 100px;text-align: center;">
			        		...
			        		<option value="4" <c:if test="${pd.category=='4' }">selected</c:if>>器械</option>
			        		<option value="5" <c:if test="${pd.category=='5' }">selected</c:if>>保健</option>
			        	</select>
			        </li>
	        	</c:otherwise>
	        </c:choose>
            ...
            <div class="weui-cells weui-cells_form" style="margin:0;font-size:16px;">
                <div class="weui-cell" style="padding:0;">
                    ...
                    <div class="weui-cell__bd">
                        <input class="weui-input" style="margin-left: 1%;" id="start" type="text" name="city" placeholder="湖北省 武汉市 武昌区" value="${pd.city }"/>
                    </div>
                </div>
            </div>
        </li>
...
<script src="static/zhaoyiwang/lib/fastclick.js"></script>
...
<script src="static/zhaoyiwang/js/city-picker.js"></script>
<script>
    $("#start").cityPicker({
        title: "选择出发地",
        onChange: function (picker, values, displayValues){
