...
<div class="searchBox" style="border-bottom: 1px solid #dedede;position: fixed;top:0;z-index: 300;">
    <div class="searchBlock">
        <form action="api/zywkehu/goDoctor.do" method="post">
...
<div class="main" style="width:100%;position: fixed;top:45px;">
    ...
    <c:forEach items="${yishengList}" var="yisheng">
        <div class="ys_detail" onclick="getDetail('${kehu_userId}','${yisheng.user_shanghu_id}','${kehu_userName}','${yisheng.youName}','医生')" id="ysdiv">
            ...
            <ul class="ys">
                <li>
                    <a href="javascript:void(0)"><b>${yisheng.youName }</b> | <span>${yisheng.position }</span></a>
                </li>
                ...
                <li>
                    ...
                    </a><!-- ${yisheng.num } ${fn:substring(yisheng.num ,3,2)}-->
...
/*   $(function(){
    ...
    //调数据重新渲染
    function doPostRender(adrs,id,keshiName){
    	var url = "api/zywkehu/getDoctorListByAddress.do";
    	var name="医生";
    	//让原来的内容隐藏
		$(".ys_info").empty();
    	 $.ajax({
            type: "post",
            dataType: "json",
            data:{address:adrs,category:id,keshiName:keshiName,identity:1},
            url: url,
            success: function (data) {
             	console.log(data.doctorList);
             	if(data.doctorList.length>0){
...
<script>
    ...
    function getkeleiListByParentID(kelei_id){
   	 var url = "api/zywkehu/getkeleiListByParentID.do";
    	//让原来的内容隐藏
		 $(".right_la ul").empty();
    	 $.ajax({
            type: "post",
    ...
    function chooseKelei(e1){
    		...
    		//把选择好的值赋值到上面
             ...
             $(".shi02").css("color","#333");
             $(".type1>a>b>i").css("background","url(<%=basePath%>static/zhaoyiwang/zywkehuduan/images/menu_5.png) top no-repeat");
    }
    ...
    function getDetail(kehu_userId,shanghu_id,userName,yisheng_name,name){
     ...
     //判断session是否存在 做相应的提示
		function checkSession(kehu_userId,shanghu_id,userName,yisheng_name,name){
			...
			if(respCode=="01"){
					window.location.href='<%=basePath%>/api/zywkehu/getYishengDetail.do?kehu_userId='+kehu_userId+'&yisheng_id='+shanghu_id+'&userName='+userName+'&yishengName='+yisheng_name+'&name='+name+'&identity=1';
			}else{
