...
<div class="searchBox" style="border-bottom: 1px solid #dedede;position: fixed;top:0;z-index: 300;">
    <div class="searchBlock">
        <form action="api/zywkehu/gojigou.do" method="post">
...
<div class="main" style="width:100%;position: fixed;top:45px;">
    ...
    <c:forEach items="${jigouList}" var="yisheng">
        <div class="ys_detail" onclick="getDetail('${yisheng.user_shanghu_id}','机构')" id="ysdiv">
            <div class="img3">
 ...
 <div class="sever">
    <section class="tog3" style=" background-color:#f4f4f4;">
        <div  class="menu_box box3" style="display:none;">
            <ul class="demo">
            <c:forEach items="${listType }" var="ltp">
                <li style="text-align: center;"><a href="javascript:void(0)" tid="${ltp.typeName }">${ltp.typeName }</a></li>
             </c:forEach> 
            </ul>
        </div>
    </section>
    ...
    var url = "api/zywkehu/getDoctorListByAddress.do";
		...
		$(".ys_info").empty();
    	 $.ajax({
            type: "post",
            dataType: "json",
            data:{address: adrs,identity:2,dengji:dengji},
            url: url,
            success: function (data) {
             	console.log(data.doctorList);
             	if(data.doctorList.length>0){
		            for(var i=0;i<data.doctorList.length;i++){
				        ...
				        appendHtml+='<a href="javascript:void(0)"><img src="'+data.doctorList[i].headImg+'" alt=""/></a></div>';    
				        appendHtml+='<ul class="ys">';       
				        appendHtml+='<li><a href="javascript:void(0)">'+data.doctorList[i].youName+'<b></b> | <span>'+data.doctorList[i].dengji+'</span></a></li>';
       ...
       /* $(".type").click(function(){
        ...
        $(".type").click(function(){
         ...
         $(".demo li").click(function(){
            	...
            	var dengji =$(this).find("a").attr("tid");
            	var address = $("#hd_sheng").val()+" "+$("#hd_city").val();
            		console.log(dengji);
            		console.log(address);
            	doPost(address,dengji);
    ...
    function getDetail(id,name){
     ...
     //判断session是否存在 做相应的提示
		function checkSession(id,name){
			...
			if(respCode=="01"){
					window.location.href='<%=basePath%>/api/zywkehu/getYishengDetail.do?yisheng_id='+id+'&name='+name+'&identity=2';
			}else{
				layer.open({
			    	  content:'您还没有登录，马上去？'
			    	  ,btn:['确定','取消']
