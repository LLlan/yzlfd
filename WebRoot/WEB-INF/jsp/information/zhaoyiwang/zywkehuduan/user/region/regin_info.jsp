...
<div id="header" style="background-color: #38c4ff;">
    <h1 class="title">${name }主页</h1><!-- api/zywkehu/goDoctor.do -->
            ...
            <c:forEach items="${fuwuList}" var="fuwu">
                <div class="bg-smallbox">
                    <div class="smallbox">
                        ...
                        <div class="ziLiao">
                            <p>${fuwu.youName }<span>${fuwu.position }</span></p>
                            <p>${fuwu.mark}</p>
...
<%-- end --%>
<form action="api/order/toRegin_pay.do" method="post" id="myForm">
...
<script type="text/javascript">
	 $(".success_case>ul>li>img").click(function(){
 ...
 <script type="text/javascript">
 	  ...
 	  var identity;
	 	  ...
	 	  var name="${name}";
	 	  if(name=="医生"){
	 		  $.post('api/zywkehu/updateLxrs.do',{user_shanghu_id:shanghu_id},function(data){
	      ...
	      location.href="<%=basePath%>api/zywkehu/myqq.do?kehu_userId="+kehu_id+"&shanghu_userId="+shanghu_id+"&userName="+userName+"&shanghuName="+shanghuName+"&identity="+identity;    
    		
    }
    
    
    function pingjiajilu(id){
    	location.href="api/zywkehu/pingjiajilu.do?user_shanghu_id="+id;
    }
	...
	function checkSession(id){
		...
		if(respCode=="01"){
			...
			window.location.href='api/order/toRegin_pay.do?fuwu_id='+id+"&WIDshow_url="+WIDshow_url;
		}else{
			 	 layer.confirm('您还没有登录，马上去？', {
 			                btn: ['确定']['取消'],
 			                title:'温馨提示',
