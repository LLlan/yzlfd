...
#lookBaseInformation table{margin: 10px 10px 0 10px;font-family: cursive;font-size: 14px;width: 780px;height: 368px}
.secondtd{padding: 0 0 0 10px;}
.firsttd{width: 70px;}
.szxl{
	background-color: #0b8be7;
    color: white;
    width: 62px;
    height: 26px;
    line-height: 26px;
...
<div class="row-fluid">
	<!-- 检索  -->
	<form action="<%=basePath%>api/shangHu/getyishenglistPage.do?identity=${msg }" method="post" id="myForm">
	<table>
		<tr>
			<td>
			...
			<td style="vertical-align:top;">
				<select onchange="abc(this,'${msg }');" name="renZhengState" style="width: 120px;">
				...
				<td style="vertical-align:top;">
					<select onchange="abcd(this,'${msg }');" name="sortValue" style="width: 120px;">
						<option value="">默认排序</option>
						<option value="1" <c:if test="${pd.sortValue=='1' }">selected="selected"</c:if>>销售量降序</option>
						<option value="2" <c:if test="${pd.sortValue=='2' }">selected="selected"</c:if>>销售量升序</option>
						<option value="3" <c:if test="${pd.sortValue=='3' }">selected="selected"</c:if>>咨询量降序</option>
						<option value="4" <c:if test="${pd.sortValue=='4' }">selected="selected"</c:if>>咨询量升序</option>
					</select>
				</td>
				<!--<c:if test="${msg eq '1' }">-->
			<!--</c:if>-->
	...
	<table id="table_report" class="table table-striped table-bordered table-hover">
		...
		<tr>
			...
			<th class="center"  style="width: 15px;">序号</th>
			...
			<th class='center'>收益明细</th>
			<th class='center'>置顶状态</th>
			<th class='center'>咨询量</th>
		...
		<c:choose>
			<c:when test="${not empty list}">
				...
				<tr>
					...
					<td class='center' style="width: 15px;">${vs.index+1}</td>
					...
					<td class='center'>
						<c:if test="${var.renZhengState=='0' }"><div style='background-color: red; color: white;'>失败</div></c:if>
						<c:if test="${var.renZhengState=='1' }"><div style='background-color: #15c300; color: white;'>通过</div></c:if>
					...
					<td class='center'><a href="javascript:void(0)" onclick="shouyimingxi('${var.user_shanghu_id }');">查看</a></td>
					<td class='center'>
					...
					<!--
					<c:if test="${msg eq '1' }">
						<td class='center'>${var.lxrs}</td>
						<td class='center' id="${var.user_shanghu_id }">${var.xsNum}</td>
					</c:if>-->
					<td class='center'>${var.lxrs}</td>
						<td class='center' id="${var.user_shanghu_id }">${var.xsNum}</td>
					<td style="width: 80px;">
					...
					<td style="width: 68px;">
						...
						<div class="szxl" onclick='szxl("${var.user_shanghu_id }")'>设置销量</div>
						<!--<c:if test="${msg eq '1' }">
							<div class="szxl" onclick='szxl("${var.user_shanghu_id }")'>设置销量</div>
						</c:if>-->
					</td>
...
<script type="text/javascript">
	...
	function szxl(id){
		//prompt层
		layer.prompt({title: '输入正确数字，并确认', formType: 3}, function(pass, index){
			if(isNaN(pass)){
				layer.msg('输入内容格式不正确',{
					time:2000
				});
			}else{
				if((parseFloat(pass) - parseInt(pass, 0)) == 0){
					$.post('api/shangHu/szxl.do',{xssl:pass,user_shanghu_fid:id,identity:identity},function(data){
						layer.close(index);
						$("#"+id).html(pass);
						layer.msg('修改成功',{
							time:2000
	...
	function abc(obj,identity){
		...
		//window.location.href="api/shangHu/getyishenglistPage.do?identity="+identity+"&renZhengState="+renZhengState;
		$("#myForm").submit();
	}
	function abcd(obj,identity){
		console.log($(obj).val());
		var sortValue=$(obj).val();
		//window.location.href="api/shangHu/getyishenglistPage.do?identity="+identity+"&sortValue="+sortValue;
		$("#myForm").submit();
	}
	...
	function topp(tagID,topp,msg){
		bootbox.confirm(msg, function(result) {
			if(result) {
				top.jzts();
				var url = '<%=basePath%>api/shangHu/topShangHu.do?user_shanghu_id='+tagID+'&top='+topp+'&identity='+identity;
		...
		bootbox.confirm(msg,function(result){
			if(result){
				...
				if(str==''){
					...
					$.ajax({
						type:'post',
						url:'<%=basePath%>api/shangHu/yishengChenked.do',
						dataTyoe:'json',
						data:{
							"ids":str,
	...
	function changrenzhengstate(obj){
		...
		if(value=='1'){
			...
			bootbox.confirm(msg, function(result) {
				if(result) {
					top.jzts();
					$.post('<%=basePath%>api/shangHu/yishengChenked.do',{ids:ids,renZhengState:renZhengState,identity:identity},function(data){
						nextPage('${page.currentPage}');
					});
				}
			});
	...
	function changstate(obj){
		...
		bootbox.confirm(msg, function(result) {
			if(result) {
				top.jzts();
				$.post('<%=basePath%>api/shangHu/yishengStatus.do',{id:id,status:status},function(data){
					nextPage('${page.currentPage}');
				});
			}
		});
	...
	function lookBaseInformation(id){
		//获取基本信息
		$.post('api/shangHu/getBaseInformation.do',{user_shanghu_id:id,identity:identity},function(data){
			var str='';
			var pd=data.pd;
			...
			str+='<tr>';
			str+='<td>积分：</td>';
			str+='<td class="secondtd">'+pd.integral+'</td>';
			str+='</tr>';
			
			str+='<tr>';
			str+='<td>虚拟币：</td>';
			str+='<td class="secondtd">'+pd.xunibi+'</td>';
			str+='</tr>';
			
			str+='<tr>';
			str+='<td>股权：</td>';
			str+='<td class="secondtd">'+pd.guquan+'</td>';
			str+='</tr>';
			
			str+='<tr>';
			str+='<td>账户余额：</td>';
			str+='<td class="secondtd">'+pd.xianjinAmount+'</td>';
			str+='</tr>';
			
			str+='<tr>';
			str+='<td>注册时间：</td>';
			str+='<td class="secondtd">'+pd.registerTime+'</td>';
			str+='</tr>';
			
			str+='<tr>';
			str+='<td>最近登录时间：</td>';
			str+='<td class="secondtd">'+pd.last_login_time+'</td>';
			str+='</tr>';
			
			str+='<tr>';
			str+='<td>最近登录IP：</td>';
			str+='<td class="secondtd">'+pd.ip+'</td>';
			str+='</tr>';
			
			str+='<tr>';
			str+='<td>认证申请时间：</td>';
			str+='<td class="secondtd">'+pd.renZhengApplyTime+'</td>';
			str+='</tr>';
			
			str+='<tr>';
			str+='<td>认证反馈时间：</td>';
			if(pd.renZhengBackTime==''){
				str+='<td class="secondtd">等待平台审核</td>';
			}else{
				str+='<td class="secondtd">'+pd.renZhengBackTime+'</td>';
			}
			str+='</tr>';
	...
	function lookRenZhengInformation(id){
		$.post('api/shangHu/getRenZhengInformation.do',{user_shanghu_id:id,identity:identity},function(data){
	...
	//查看指定商家的收益
	function shouyimingxi(id){
		//获取基本信息
		$.post('api/shangHu/getmingxiOfshangjia.do',{user_shanghu_id:id},function(data){
			var str='';
			var pd=data.pd;
			str+='<div id="lookBaseInformation" style="display: none;">';
			...
			if(data.shouyi!=0){
				str+='<tr>';
				str+='<td class="firsttd">总收益（元）：</td>';
				str+='<td class="secondtd" colspan="4">'+parseFloat(data.shouyi).toFixed(2)+'</td>';
				str+='</tr>';
				
				str+='<tr>';
				str+='<td class="firsttd">订单时间</td>';
				str+='<td class="firsttd">服务（产品）名称</td>';
				str+='<td class="firsttd">该单总额（元）</td>';
				str+='<td class="firsttd">平台收费（元）</td>';
				str+='<td class="firsttd">该单收益（元）</td>';
				str+='</tr>';
				
				$.each(data.list,function(){
					str+='<tr>';
					str+='<td class="secondtd">'+this.create_order_time+'</td>';
					str+='<td class="secondtd">'+this.fuwu_name+'</td>';
					str+='<td class="secondtd">'+parseFloat(this.totalPrice).toFixed(2)+'</td>';
					str+='<td class="secondtd">'+parseFloat(this.pingtaiPrice).toFixed(2)+'</td>';
					str+='<td class="secondtd">'+parseFloat(this.shanghuPrice).toFixed(2)+'</td>';
					str+='</tr>';
				});
			}else{
				str+='<tr>';
				str+='<td class="firsttd">总收益：</td>';
				str+='<td class="secondtd" colspan="3">该商家尚未产生收益</td>';
				str+='</tr>';
			}
			...
			layer.open({
		        type: 1
		        ,title: "商家收益详情" //不显示标题栏   title : false/标题
		        ,closeBtn: 1//是否显示关闭按钮0和1
		        ,area:['800px', '600px']//宽高
		        ,shade: 0.5//透明度
		        ,id: 'LAY_layuipro' //设定一个id，防止重复弹出
