...
<body>
	<form action="api/shangHu/${msg }.do" method="post" id="Form" enctype="multipart/form-data">
		...
		<table style="margin: 15px 0 0 25px;">
			<tr class="info">
				...
				<td>
					<select name="type">
						<option value="1" <c:if test="${pd.type=='1' }">selected="selected"</c:if>>华夏郎中行</option>
						<option value="2" <c:if test="${pd.type=='2' }">selected="selected"</c:if>>旅居养老</option>
						<option value="3" <c:if test="${pd.type=='3' }">selected="selected"</c:if>>古玩字画苑</option>
					</select>
				</td>
			</tr>
			<tr class="info">
