                    ...
                    <c:choose>
                    	<c:when test="${list.type=='3' }">
	                    	<div class="bg-smallbox" style="margin-top:8px;height: 310px;margin-bottom: 10px;">
			                    ...
			                    <div class="smallbox">
			                        <img src="static/lvjuyanglao/images/ill.png" style="width: 20%;margin-top: 30px;"/>
			                        <div class="ziLiao">
			                            <p>${list.realName }|特产</p>
			                            ...
			                            <div style="color: #6f6f6f;font-size: 13px;height: 41px;width: 100%;overflow: hidden;text-overflow: ellipsis;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;">产品名称：<span>${list.fuwu_name }</span></div>
			                            <p style="width: 100%">单价：<span style="float: right;">￥${list.price }</span></p>
			                            <p style="width: 100%;color: black;">数量：<span style="float: right;">${list.number }</span><span style="float: right;font-size: 1px;margin-top: 4px">X</span></p>
			                            <p style="width: 100%">总价：<span style="float: right;">￥${list.kehuPrice }</span></p>
			                        </div>
			                    </div>
			                    <%--
                    	...
                    	<c:otherwise>
                    		<div class="bg-smallbox" style="margin-top:11px;">
				                <div class="smallbox">
				                    <img src="static/lvjuyanglao/images/ill.png"/>
				                    <div class="ziLiao">
			                    		<p>${list.realName }|${list.type=="1"?"酒店":"景点" }<span></span><span></p>
				                        <p><span>${list.address }</span></p>
				                        <p>产品名称:${list.fuwu_name }</p>
				                        <p>价格:${list.kehuPrice }元</p>
				                        <a href="javascript:void(0)" class="yijiuzhen" style="background-color:#fff;color:#000;font-size:14px;border:1px solid #eee;padding:6px 3%;position: absolute;top:12px;right:0;border-radius: 5px;">进行中</a>
				                        <a href="javascript:void(0);" class="quxiao01" onclick="lianxihuanzhe('${list.user_kuhu_id}','${list.realName }','${list.user_shanghu_id }','${list.youName }')">联系买家</a>
				                     </div>
		                		</div>
	           				</div>
                    	</c:otherwise>
...
function lianxihuanzhe(k_id,k_name,s_id,s_name){
	...
	console.log(s_id)
	console.log(s_name)
	location.href="<%=basePath%>api/ljylkehu/lianxihuanzhe.do?ku_id="+k_id+"&kehu_name="+k_name+"&s_id="+s_id+"&s_name="+s_name;    
}
    ...
    li.click(function(){
        ...
        $.post('api/ljylshangHu/personMyOrderYesOrNo.do',{order_state:order_state},function(data){
        	if(data.reqCode=="01"){
        		var str='';
        		$.each(data.list,function(){
        			if(this.type=='3'){
        				...
        				str+='<img src="static/lvjuyanglao/images/ill.png" style="width: 20%;margin-top: 30px;"/>';
        				str+='<div class="ziLiao">';
        				str+='<p>'+this.realName+'|特产</p>';
        				...
        				str+='<div style="color: #6f6f6f;font-size: 13px;height: 41px;width: 100%;overflow: hidden;text-overflow: ellipsis;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;">产品名称：<span>'+this.fuwu_name+'</span></div>';
        				str+='<p style="width: 100%">单价：<span style="float: right;">￥'+this.price+'</span></p>';
        				str+='<p style="width: 100%;color: black;">数量：<span style="float: right;">'+this.number+'</span><span style="float: right;font-size: 1px;margin-top: 4px">X</span></p>';
        				str+='<p style="width: 100%">总价：<span style="float: right;">￥'+this.kehuPrice+'</span></p>';
	                    str+='</div>';       
	                    str+='</div>';       
            			...
            			str+='<div class="smallbox">';
            			str+='<img src="static/lvjuyanglao/images/ill.png"/>';
            			str+='<div class="ziLiao">';//+'this.type=="1"?"酒店":"景点"'+
            			//str+='<p>'+this.realName;
            			if(this.type=="1"){
            				str+='<p>'+this.realName+'|酒店<span></span><span></span><span></span></p>';
            			}else{
            				str+='<p>'+this.realName+'|景点<span></span><span></span><span></span></p>';
            			}
            			
            			if(order_state=='1'){
            				str+='<p>完成时间：<span>'+this.complete_time+'</span></p>';
            			}
            			str+='<p>产品名称:'+this.fuwu_name+'</p>';
            			str+='<p>价格:'+this.kehuPrice+'元</p>';
            			if(order_state=='1'){
            				str+='<a href="javascript:void(0)" style="background-color:#fff;color:#000;font-size:14px;border:1px solid #eee;padding:6px 4%;position: absolute;top:12px;right:0;border-radius: 5px;">已完成</a>';
            				str+="<a href=\"javascript:void(0)\" onclick=\"lianxihuanzhe('+this.user_kuhu_id+','+this.realName+','+this.user_shanghu_id+','+this.youName+')\" class=\"quxiao01\" style=\"background-color:#fff;color:#fdb836;font-size:14px;border:1px solid #eee;padding:6px 4%;position: absolute;top:70px;right:0;border-radius: 5px;\">联系买家</a>";
            			}else{
            				str+='<a href="javascript:void(0)" class="yijiuzhen" style="background-color:#fff;color:#000;font-size:14px;border:1px solid #eee;padding:6px 3%;position: absolute;top:12px;right:0;border-radius: 5px;">进行中</a>';
            				str+="<a href=\"javascript:void(0);\" class=\"quxiao01\" onclick=\"lianxihuanzhe('+this.user_kuhu_id+','+this.realName+','+this.user_shanghu_id+','+this.youName+')\">联系买家</a>";
            			}
            			str+='</div>';
            			str+='</div>';
            			str+='</div>';
