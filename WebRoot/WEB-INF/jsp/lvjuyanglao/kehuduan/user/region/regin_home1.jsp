    ...
    <style>
       ...
       /* a{
        ...
        .xuansheng>a>b>i,.type01{
            display:block;width:.75rem;height:.38rem;margin-left:75%;margin-top:-16px;background: url(<%=basePath%>static/lvjuyanglao/zywkehuduan/images/menu_5.png) no-repeat;background-size:.75rem auto;
        }
...
<div id="header" style="background-color: #38c4ff;">
    ...
    <a href="api/ljylkehu/index.do" class="icon1"><i class="icon-angle-left "></i></a>
</div>
<%--
<div class="searchBox" style="border-bottom: 1px solid #dedede;position: fixed;top:0;z-index: 300;">
    <div class="searchBlock">
        <form action="api/ljylkehu/goDoctor.do" method="post">
...
--%>
<div class="main" style="width:100%;position: fixed;top:0px;">
    <div class="menu">
        <ul class="regin">
            <li class="xuansheng">
                <a href="javascript:void(0)"><b><span class="shi02">按地区搜索</span><i class="type01"></i></b></a>
            </li>
            <%--
            <li class="type">
            ...
            <li class="type1">
                <a href="javascript:void(0)"><b><span class="shi02" id="kelei">科类</span><i class="type01"></i></b></a>
            </li>
            --%>
        </ul>
    </div>
    ...
    <c:forEach items="${yishengList}" var="yisheng">
        <div class="ys_detail" onclick="getDetail('${kehu_userId}','${yisheng.user_shanghu_id}','${kehu_userName}','${yisheng.youName}','医生')" id="ysdiv">
            <div class="img3">
                <a href="javascript:void(0)"><img src="${yisheng.headImg }" alt=""/></a>
            </div>
            <ul class="ys">
                <li>
                    <a href="javascript:void(0)"><b>${yisheng.youName }</b> <span></span></a>
                </li>
    ...
    <section class="tog3" style=" background-color:#f4f4f4;">
        <div  class="menu_box box3" style="display:none;">
            <ul class="demo" style="text-align: center">
                <li><a href="javascript:void(0)" tid="1">中医</a></li>
                <li><a href="javascript:void(0)" tid="2">西医</a></li>
                <li><a href="javascript:void(0)" tid="3">中西医</a></li>
                <li><a href="javascript:void(0)" tid="4">疑难杂症</a></li>
            </ul>
        </div>
    </section>
    <section class="tog2">
        <div class="menu_box box2" style="display:none;">
            <div class="left_la">
                <div class="wrap wrap3">
                    <ul >
                    <c:forEach items="${ParentkeleiList }" var="kelei">
                        <li value="2"><a href="javascript:;" tid="${kelei.kelei_id}">${kelei.keleiName }</a></li>
					</c:forEach>
                    </ul>
                </div>
            </div>
            <div class="right_la" >
                <div class="wrap wrap4" >
                    <ul>
                        <!-- <li value="2"><a href="javascript:void(0);">上海</a></li> -->
                    </ul>
                </div>
            </div>
        </div>
...
<script>
    $(function () {
        ...
        //给内容div设置高度
		$(".ys_info").height($(window).height()-95);
...
/*   $(function(){
       $(".xuansheng").on("click",function(){
           if($("#sheng").css("display","block")){
               $(".xuansheng>a>b>i").css({"background":"url(<%=basePath%>static/lvjuyanglao/zywkehuduan/images/menu_5.png) bottom no-repeat"});
               //$(".ys_info").show();
               //$("#sheng").show();
               $(".mask").show();
               $("#city").show();
             ...
             //  alert("111");
               $(".xuansheng>a>b>i").css({"background":"url(<%=basePath%>static/lvjuyanglao/zywkehuduan/images/menu_5.png) top no-repeat"});
               //$("#sheng").hide();
               $(".mask").hide();
               $("#city").hide();
           }
    ...
    window.onload=function(){//当页面加载完成执行  html css js
        $(".xuansheng").click(function(){
            $(".tog2").hide();
            $(".tog3").hide();
            $(".type1").find("a>b>i").css("background","url(<%=basePath%>static/lvjuyanglao/zywkehuduan/images/menu_5.png) top no-repeat");
            $(".type").find("a>b>i").css("background","url(<%=basePath%>static/lvjuyanglao/zywkehuduan/images/menu_5.png) top no-repeat");
            $(".shi02").css("color","#333");
    ...
    function getName(m1){
		...
		//让原来的内容清空
		$(".ys_info").empty();
		var url = "api/ljylkehu/getDoctorListByAddress.do";
		...
		var type = "${pd.type}";
		 $.ajax({
            type: "post",
            dataType: "json",
            data:{address: adrs,type:type},
            url: url,
            success: function (data) {
             	console.log(data.doctorList);
             	if(data.doctorList.length>0){
		            for(var i=0;i<data.doctorList.length;i++){
						var appendHtml = '<div class="ys_detail" onclick="getDetail(\''+data.kehu_userId+'\',\''+data.doctorList[i].user_shanghu_id+'\',\''+data.kehu_userId+'\',\''+data.doctorList[i].youName+'\',\''+name+'\')">';
				        ...
				        appendHtml+='<li><a href="javascript:void(0)">'+data.doctorList[i].youName+'<b></b>  <span></span></a></li>';
				        appendHtml+='<li><a href="javascript:void(0)"></a></li>';
...
<script>
    $(function(){
       /* $(".type").click(function(){
            ...
            $(".type1").find("a>b>i").css("background","url(<%=basePath%>static/lvjuyanglao/zywkehuduan/images/menu_5.png) top no-repeat");
            $(".shi02").css("color","#333");
            $(".xuansheng").find("a>b>i").css("background","url(<%=basePath%>static/lvjuyanglao/zywkehuduan/images/menu_5.png) top no-repeat");
            $("#sheng").hide();
            $("#city").hide();
            ...
            if($(".box1").hasClass("have")){
                ...
                $(this).find("a>b .shi02").css("color","#333");
                $(this).find("a>b>i").css("background","url(<%=basePath%>static/lvjuyanglao/zywkehuduan/images/menu_5.png) top no-repeat");
            }else{
                $(".mask").show();
                $(".sever>section").eq(index-1).show().siblings().hide();
                $(".box1").css("display","block").addClass("have");
                $(this).find("a>b .shi02").css("color","#54ccb4");
                $(this).find("a>b>i").css("background","url(<%=basePath%>static/lvjuyanglao/zywkehuduan/images/menu_5.png) bottom no-repeat");
            }
            //$(".sever>section").eq(index-1).show().siblings().hide()
        ...
        */
        $(".type").click(function(){
         ...
         $(".demo li").click(function(){
                ...
                $(".shi02").css("color","#333");
                $(".type>a>b>i").css("background","url(<%=basePath%>static/lvjuyanglao/zywkehuduan/images/menu_5.png) top no-repeat");
            	var type_id = $(this).find("a").attr("tid");
            	var address = $("#hd_sheng").val()+" "+$("#hd_city").val();
            	
            	//console.log($(this).find("a").attr("tid"));
            	...
            	doPostRender(address);
            });
        
        /*选择科类点击事件*/
        $(".type1").click(function(){
        	//给第一个元素默认背景色
       	 	$(".left_la ul li").eq(0).addClass("active");
        	//获取到第一个元素的值
      		//console.log($(".left_la ul li").eq(0).find("a").attr("tid"));
      		var kelei_id = $(".left_la ul li").eq(0).find("a").attr("tid");
      		
      		//点击科类选项展开时默认查询第一个元素的子类--调后台--找子类
      		getkeleiListByParentID(kelei_id);
    ...
    $(".left_la ul li").click(function(){
    	...
    	console.log($(this).find("a").attr("tid"))
    	var kelei_id = $(this).find("a").attr("tid");
    	getkeleiListByParentID(kelei_id);
    	
    });
    
    //根据父类id查出子类列表
    function getkeleiListByParentID(kelei_id){
   	 var url = "api/ljylkehu/getkeleiListByParentID.do";
    	//让原来的内容隐藏
		 $(".right_la ul").empty();
    	 $.ajax({
            type: "post",
            dataType: "json",
            data:{kelei_id:kelei_id},
            url: url,
            success: function (data) {
	             	if(data.keleiList.length>0){
			            for(var i=0;i<data.keleiList.length;i++){
							var appendHtml = '<li value="2" onclick="chooseKelei(this)" tid="'+data.keleiList[i].keleiName+'"><a href="javascript:void(0);">'+data.keleiList[i].keleiName+'</a></li>';
					        $(".right_la ul").append(appendHtml);
			        	}
		        	}else{
		        			/* var appendHtml = '<div class="ys_detail2">暂无相关数据</div>';
					         $(".right_la ul").append(appendHtml); */
    ...
    //选择子科类
    function chooseKelei(e1){
   			var type_id = $("#type_id").val();
           	var address = $("#hd_sheng").val()+" "+$("#hd_city").val();
           	var keshiName = $(e1).attr("tid");
           	console.log("111111-----"+address);
           	console.log("222222-----"+type_id);
           	console.log("333333-----"+keshiName);
           	doPostRender(address,type_id,keshiName);
    
    		//把选择好的值赋值到上面
  			 $(".type1>a>b>span").html($(e1).attr("tid"));
             $(".mask").hide();
             $(".tog2").css("display","none");
             $(".shi02").css("color","#333");
             $(".type1>a>b>i").css("background","url(<%=basePath%>static/lvjuyanglao/zywkehuduan/images/menu_5.png) top no-repeat");
    }
    
    
    
    //点击div查看医生详情
    function getDetail(kehu_userId,shanghu_id,userName,yisheng_name,name){
		checkSession(kehu_userId,shanghu_id,userName,yisheng_name,name);
    }
    
     //判断session是否存在 做相应的提示
		function checkSession(kehu_userId,shanghu_id,userName,yisheng_name,name){
			var respCode = "${respCode}";
			if(respCode=="01"){
					window.location.href='<%=basePath%>/api/ljylkehu/getYishengDetail.do?kehu_userId='+kehu_userId+'&yisheng_id='+shanghu_id+'&userName='+userName+'&yishengName='+yisheng_name+'&name='+name+'&type=${pd.type}';
			}else{
				layer.open({
			    	  content:'您还没有登录，马上去？'
			    	  ,btn:['确定','取消']
