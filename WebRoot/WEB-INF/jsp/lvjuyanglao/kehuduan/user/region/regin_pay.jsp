...
<div class="main" style="margin-top: 50px;">
    ...
    <div class="weixinZ" style="height: 60px;margin-bottom: 10px;">
        <p style="top: 7px;">
            <img src="static/lvjuyanglao/zywkehuduan/images/weixin_03.png" alt=""/>
    ...
    <div class="weixinZ" style="height: 60px;">
        <p style="top: 7px;">
            <img src="static/lvjuyanglao/zywkehuduan/images/zfb.png" alt=""/>
...
<script>
	...
	function querenzhifu(){
		...
		if($(".check1 input").attr("checked")=="checked" || $(".check2 input").attr("checked")=="checked"){
			...
			document.getElementById("WIDsubject").value = "旅居养老:购买产品";//body
			...
			if (browser.versions.mobile) {//移动设备中打开
		        ...
		        if (ua.match(/MicroMessenger/i) == "micromessenger") {//在微信中打开
		        	if($(".check1 input").attr("checked")=="checked"){//选择微信支付
				    	...
				    	$("#payForm").attr("action","api/ljylAlipay/pay.do?num="+sNow);
				    	$("#payForm").submit();
				    }    
		        }else{//非微信内打开
		        	if($(".check1 input").attr("checked")=="checked"){//选择微信支付
				    	...
				    	//alert("非微信浏览器支持支付宝支付");
				    	$("#payForm").attr("action","api/ljylAlipay/alipay.do");
				    	$("#payForm").submit();
				    }  
		        }
			} else {//PC浏览器打开
        ...
        $(".check3 input").click(function(){
        	if($(".check3 input").attr("checked")=="checked"){
        		...
        		if(parseFloat($("#price").html())<10){
        			...
        			$.ajax({
        				type:"post",
        				url:'api/ljylorder/getXuNiBiNumber.do',
        				data:{},
        				dataType:"json",
