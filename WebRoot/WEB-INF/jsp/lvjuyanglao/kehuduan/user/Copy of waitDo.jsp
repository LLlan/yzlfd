...
<div class="ziliaoBox01" style="margin-top: 50px;">
    <div class="smallbox">
        <img src="static/lvjuyanglao/zywkehuduan/images/ill.png"/>
        <div class="ziLiao">
        ...
        <input type="hidden" id="rname" value="${pd.realName}"/>
            ...
            <a href="<%=basePath%>api/ljylkehu/gerenInfo.do?user_kuhu_id=${pd.user_kuhu_id}" class="ziliao01" style="padding:6px 2%;background-color: #fff;color: #333;">个人信息</a>
        </c:if>
        </div>
    </div>
</div>
  <div class="profit">
        <div class="cash">
            <a href="api/ljylkehu/myMoney.do?user_kuhu_id=${pd.user_kuhu_id}" style="display: block;" onclick="toPersonCenterJump('2');">
                <em>${pd.money }</em>
                <p>现金金额</p>
            </a>
        </div>
        ...
        <div class="integral">
            <a href="api/ljylkehu/myJifen.do?user_kuhu_id=${pd.user_kuhu_id}" style="display: block;">
                <em>${pd.jifen }</em>
                <p>我的积分</p>
            </a>
        </div>
...
<div id="box"style="clear: both;margin-bottom:60px;">
	        ...
	        <div class="bg-smallbox" style="margin-top:11px;">
	            <div class="smallbox">
	                ...
	                <div class="ziLiao">
	                    <p>${ol.youName } 
	                    <span>
	                    <c:if test="${ol.type eq '1'}">酒店</c:if>
	                    <c:if test="${ol.type eq '2'}">景点</c:if>
	                    <c:if test="${ol.type eq '3'}">特产</c:if>
	                    </span></p>
	                    <p></p>
   ...
   <!--  <div class="hide showbox box2">
        <div class="bg-smallbox"  style="margin-top:11px;">
            <div class="smallbox">
                <img src="static/lvjuyanglao/zywkehuduan/images/yisheng_10.png"/>
                <div class="ziLiao">
...
<script type="text/javascript">
	...
	function yijiuzhen(user_kehu_id,user_shanghu_id,price,shanghu_price,order_id,type){
		  layer.confirm('您确定进行确认付款操作吗？', {
	            ...
	            closeBtn:0,
	        		...
	        		location.href='api/ljylkehu/yijiuzhen.do?user_kehu_id='+user_kehu_id+'&user_shanghu_id='+user_shanghu_id+'&price='+price+'&shanghu_price='+shanghu_price+'&order_id='+order_id+'&type='+type;
	        	}
 		  );
	...
	function qxdd(id,identity){
		  layer.confirm('您确定进行取消订单操作吗？', {
 			                ...
 			                closeBtn:0,
 			            		...
 			            		location.href='api/ljylshangHu/updateTuiDan0.do?id='+id+'&identity='+identity;
 			            	}
 			    );
	...
	function getOrderListByWhereDzx(state){
		 location.href="api/ljylkehu/huiyuanzhongxin.do?order_state=2";
	}
	
	//已完成
	function getOrderListByWhereYwc(state){
         location.href="api/ljylkehu/huiyuanzhongxin.do?order_state=1";
	}
	
	//去评价
	function qupj(tid,user_fid,user_shanghu_id,identity){
		location.href="api/ljylkehu/gopj.do?tid="+tid+"&user_fid="+user_fid+"&user_shanghu_id="+user_shanghu_id+"&identity="+identity;
	}
...
<script>
	$(function(){
     	...
     	if($("#rname").val()==""){
				   layer.confirm('您还未完善资料，马上去？', {
  			                ...
  			                closeBtn:0,
  			            			...
  			            			window.location.href='api/ljylkehu/gerenziliao.do';
  			            	}
  			       ); 
     	}
     }
	 ...
	 //退出
	 function tuichu(){
	 		window.location.href='api/ljylkehu/tuichu.do';
	 }
