...
<script type="text/javascript">
	...
	function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>api/ljylshangHu/toAddNewsAndNotice.do?category='+category;
		 diag.Width = 720;
		 diag.Height = 500;
	...
	function edit(tagID){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑";
		 diag.URL = '<%=basePath%>api/ljylshangHu/toEditNewsAndNotice.do?newsandnotice_id='+tagID;
		 diag.Width = 720;
		 diag.Height = 500;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	function topp(tagID,topp,msg){
		bootbox.confirm(msg, function(result) {
			if(result) {
				top.jzts();
				var url = '<%=basePath%>api/ljylshangHu/topNewsAndNotice.do?newsandnotice_id='+tagID+'&top='+topp;
	...
	function del(tagID){
		bootbox.confirm("确定要删除该条数据吗?", function(result) {
			if(result) {
				top.jzts();
				var url = '<%=basePath%>api/ljylshangHu/deleteNewsAndNoticeById.do?ids='+tagID;
				$.get(url,function(data){
					nextPage('${page.currentPage}');
				});
			}
		...
		bootbox.confirm('确定执行批量删除操作吗？',function(result){
			if(result){
				...
				if(str==''){
					...
					$.ajax({
						type:'post',
						url:'<%=basePath%>api/ljylshangHu/deleteNewsAndNoticeById.do',
						dataTyoe:'json',
						cache: false,
						data:{
							"ids":str
	...
	function lookDetaileInformation(id){
		//获取基本信息
		$.post('api/ljylshangHu/getNewsAndNoticeDetaileInformation.do',{newsandnotice_id:id},function(data){
			var str='';
			var pd=data.pd;
...
<div class="row-fluid">
	<!-- 检索  -->
	<form action="<%=basePath%>api/ljylshangHu/getnewsandnoticelistPage.do" method="post">
