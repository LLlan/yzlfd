...
<div class="row-fluid">
	<!-- 检索  -->
	<form action="<%=basePath%>api/ljylorder/getOrderOfYaoShanglistPage.do" method="post" id="myForm">
	<table>
		<tr>
			<td>
		...
		<c:choose>
			<c:when test="${not empty list}">
				...
				<tr>
					...
					<td class='center'>
						<c:choose>
							<c:when test="${var.complete_time=='' || var.complete_time==null }">尚未完成</c:when>
							<c:otherwise>${var.complete_time }</c:otherwise>
						</c:choose>
					</td>
					<td class='center'>
						<c:choose>
							<c:when test="${var.order_state=='1' }"><span style="color:#00cd66">已完成</span></c:when>
							<c:when test="${var.order_state=='2' }">进行中</c:when>
