		...
		//if(verify_result){//验证成功
			if(trade_status.equals("TRADE_FINISHED") || trade_status.equals("TRADE_SUCCESS")){
		        ...
		        if(xnbNum>0){
		        	...
		        	PageData pd2=keHuService.getDetailByID(p1);
		        	...
		        	keHuService.updateKeHuXuNiBi(p1);
		        }
				...
				if(tempfuwu!=null){
					if(tempfuwu.getString("type").equals("1")){//1-医生2.机构3-药商
						...
						tempyis.put("price", tempfuwu.get("price").toString());
						...
						tempyis.put("xnbNum", xnbNum);
						tempyis.put("shanghuPrice", df.format(Float.parseFloat(tempfuwu.get("price").toString())*(1-bili)));
						tempyis.put("kehuPrice", Float.parseFloat(tempfuwu.get("price").toString())-xnbNum);
						...
						tempjig.put("price", tempfuwu.get("price").toString());
						...
						tempjig.put("xnbNum", xnbNum);
						tempjig.put("shanghuPrice", df.format(Float.parseFloat(tempfuwu.get("price").toString())*(1-bili)));
						tempjig.put("kehuPrice", Float.parseFloat(tempfuwu.get("price").toString())-xnbNum);
	...
	public ModelAndView return_url(HttpServletRequest request,HttpSession session) throws Exception{
		...
		//if(verify_result){//验证成功
			if(trade_status.equals("TRADE_FINISHED") || trade_status.equals("TRADE_SUCCESS")){
				...
				PageData tempData = keHuService.getUserKehuIdByOrderNumber(spd);
