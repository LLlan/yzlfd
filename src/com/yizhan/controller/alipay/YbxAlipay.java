 ...
 */
@Controller
@RequestMapping("/api/ybxAlipay")
public class YbxAlipay extends BaseController{
	//double保留两位小数
    ...
    public ModelAndView payConfirm(HttpServletRequest request,HttpSession session) throws Exception { 
		logBefore(logger, "--唤起手机支付宝APPybx--");
		...
		if(keHu!=null){
			...
			sParaTemp.put("notify_url", AlipayConfig.notify_url_ybx);
			sParaTemp.put("return_url", AlipayConfig.return_url_ybx);
	...
	public void notify_url(HttpServletRequest request,HttpServletResponse response) throws Exception{
		logBefore(logger, "--支付成功回调（异步通知）ybx--");
		//获取支付宝POST过来反馈信息
		...
		//if(verify_result){//验证成功
			if(trade_status.equals("TRADE_FINISHED") || trade_status.equals("TRADE_SUCCESS")){
		        ...
		        if(xnbNum>0){
		        	...
		        	PageData pd2=ybxKeHuService.getDetailByID(p1);
		        	...
		        	ybxKeHuService.updateKeHuXuNiBi(p1);
		        }
				
				//获取医生的服务
				PageData tempfuwu=ybxShangHuService.getDataByIdOfService(pd);
				//获取平台技术服务扣除比例对象
	...
	public ModelAndView return_url(HttpServletRequest request,HttpSession session) throws Exception{
		logBefore(logger, "--支付成功回调（同步通知）ybx--");
		ModelAndView mv=new ModelAndView();
		//获取支付宝GET过来反馈信息
		Map<String,String> params = new HashMap<String,String>();
		...
		//if(verify_result){//验证成功
			if(trade_status.equals("TRADE_FINISHED") || trade_status.equals("TRADE_SUCCESS")){
				...
				PageData tempData = ybxKeHuService.getUserKehuIdByOrderNumber(spd);
				...
				mv.setViewName("alipay/alipayreturn_ybx");
			}
			System.out.println("验证成功");
		//}else{
			//System.out.println("验证失败");
...
///////////////////////////////////////////////////以下为在微信浏览器中选择支付宝支付时的方法/////////////////////////////////////////////////////////////	
	...
	public ModelAndView pay(HttpSession session) throws Exception{
		...
		PageData tempIsOrNo=ybxOrderService.get_tb_temp_information(pd);
		if(tempIsOrNo==null){
			//获取客户信息
	      	ZywKeHu zywKeHu=(ZywKeHu) session.getAttribute(Const.SESSION_KEHU_USER);
			//保存临时信息
	      	...
	      	ybxOrderService.save_tb_temp_information1(temp);
	      	mv.setViewName("alipay/pay");
		}else{
			mv.setViewName("redirect:/api/ybxAlipay/demo_post?order_number="+pd.getString("num"));
		}
		return mv;
	}
	//在浏览器中打开，确认支付页面
