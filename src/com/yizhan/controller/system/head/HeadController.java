package com.yizhan.controller.system.head;
...
import com.yizhan.util.Watermark;
import com.yizhan.util.mail.SimpleMailSender;
			...
			if(strEM.length == 4){
				if("yes".endsWith(isAll)){
					try {
						...
						try {
							for(int i=0;i<userList.size();i++){
								if(Tools.checkEmail(userList.get(i).getString("EMAIL"))){		//邮箱格式正确就发送，不对就跳过
