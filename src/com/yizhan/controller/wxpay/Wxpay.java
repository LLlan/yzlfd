	...
	public void notify_url(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		if(map.get("return_code").equals("SUCCESS")){
			...
			if(map.get("result_code").equals("SUCCESS")){
				...
				//获取服务信息
				PageData tempfuwu=shangHuService.getDataByIdOfService(temp);
				...
				if(tempfuwu!=null){
					//1.保存订单信息
					if(tempfuwu.getString("type").equals("1")){//购买的医生服务
						PageData yisPd=orderService.getOrderOfYiShengByOrderNumber(pd);
						if(yisPd==null){//说明还未进行处理
							//保存订单信息
							...
							tempyis.put("price", df.format(Float.parseFloat(tempfuwu.get("price").toString())*(1-bili)));
							tempyis.put("pay_method", "微信");
							tempyis.put("pay_number", transaction_id);
							...
							tempyis.put("pingtaiPrice", df.format(Float.parseFloat(tempfuwu.get("price").toString())*bili));
							tempyis.put("fuwubili", biliData.getString("bili_jishufuwu"));
							
							orderService.saveOrderOfYiSheng(tempyis);
						}
					}else if(tempfuwu.getString("type").equals("2")){//机构服务
						PageData jigPd=orderService.getOrderOfJiGouByOrderNumber(pd);
						if(jigPd==null){//说明还未进行处理
							//保存订单信息
							...
							tempjig.put("price", df.format(Float.parseFloat(tempfuwu.get("price").toString())*(1-bili)));
							tempjig.put("pay_method", "微信");
							tempjig.put("pay_number", transaction_id);
							...
							tempjig.put("pingtaiPrice", df.format(Float.parseFloat(tempfuwu.get("price").toString())*bili));
							tempjig.put("fuwubili", biliData.getString("bili_jishufuwu"));
							
							orderService.saveOrderOfJiGou(tempjig);
						}
					}else{//药商服务
						PageData yaosPd=orderService.getOrderOfYaoShangByOrderNumber(pd);
